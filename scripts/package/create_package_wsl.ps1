...
if ($Help -or $args -contains '--help' -or $args -contains '-h') {
    Write-Host @"
create_package_wsl.ps1 — Build Python distribution package via WSL

Delegates to create-package.sh inside WSL Ubuntu.

Usage:
    scripts\package\create_package_wsl                         (default Ubuntu distro)
    scripts\package\create_package_wsl -WslDist Ubuntu-22.04   (specific distro)
"@
    exit 0
}
...
Write-Host "[create_package_wsl] Distro    : $WslDist"
Write-Host "[create_package_wsl] Project   : $WslPath"
Write-Host ""
...
try {
    "cd '$WslPath' && bash scripts/package/create-package.sh" | Out-File -FilePath $tmpFile -Encoding ASCII -NoNewline
    Write-Host "Starting WSL build..."
    wsl -d $WslDist -- bash $tmpWsl
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[create_package_wsl] FAILED (exit $LASTEXITCODE)"
    }
    exit $LASTEXITCODE
} finally {
